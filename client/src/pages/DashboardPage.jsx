...
import {
  ...
  MessageSquare, FileQuestion, TrendingUp, Sparkles,
  Lightbulb, Clock, Brain, BarChart2, CheckCircle2, BookMarked, Trophy
} from 'lucide-react';
...
const TIPS = [
  ...
  { icon:'🌊', tip:'Short focused bursts beat marathon sessions every time.' },
...
export default function DashboardPage() {
  ...
  const [spacedReminders, setSpacedReminders]       = useState([]);
  ...
  useEffect(() => {
    if (!user?.uid) return;
    getQuizHistoryFromFirestore(user.uid, 30).then(history => {
      setQuizHistory(history);

      // Spaced repetition: topics below 60% not studied in 3+ days
      const topicMap = {};
      history.forEach(q => {
  ...
  const loadWeeklyReport = async () => {
    ...
    setShowReport(true);
    try {
      const weekHistory = quizHistory.filter(q => {
        if (!q.timestamp) return false;
        return (Date.now() - new Date(q.timestamp).getTime()) < 7 * 86400000;
      });
      const res = await getWeeklyReport(weekHistory, profile?.streak||0, profile?.xp||0);
      setWeeklyReport(res.data.report);
    } catch { setWeeklyReport('Backend server not running. Start it to get your AI weekly report!'); }
    ...
    <div className="p-4 md:p-6 lg:p-7 space-y-4 max-w-[1400px]">
      ...
      <motion.div initial={{ opacity:0, y:-10 }} animate={{ opacity:1, y:0 }}
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button onClick={loadWeeklyReport} disabled={reportLoading}
            className="flex items-center gap-1.5 text-xs glass border border-violet-500/20 text-violet-400 rounded-full px-3 py-1.5 hover:border-violet-500/40 transition-all disabled:opacity-50 flex-shrink-0">
          ...
          <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:'auto' }} exit={{ opacity:0, height:0 }}
            ...
            <div className="glass border border-violet-500/20 rounded-2xl p-4 flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {reportLoading ? (
                  <p className="text-xs text-white/40">Analyzing your week...</p>
                ) : (
                  <p className="text-sm text-white/70 leading-relaxed">{weeklyReport}</p>
                )}
              </div>
              <button onClick={() => setShowReport(false)} className="text-white/20 hover:text-white/50 text-xs">✕</button>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {(profile?.streak||0) >= 2 && (
        <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-white/40 truncate">Best: {profile.longestStreak||0} days · Keep studying today!</p>
          </div>
          <div className="hidden sm:flex gap-0.5">
            {Array.from({ length:Math.min(profile.streak,7) }).map((_,i) => <span key={i} className="text-base">🔥</span>)}
          </div>
        </motion.div>
      )}

      {/* Spaced Repetition Reminders */}
      {spacedReminders.length > 0 && (
        <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }}
          className="glass border border-neon-amber/20 rounded-2xl p-4">
          <div className="flex items-center gap-2 mb-3">
            <AlertTriangle size={14} className="text-neon-amber" />
            <p className="text-xs font-bold text-neon-amber uppercase tracking-widest">Spaced Repetition Reminders</p>
          </div>
          <div className="flex flex-wrap gap-2">
            {spacedReminders.map((t,i) => (
              <Link key={i} to="/app/study-sessions">
                <div className="flex items-center gap-2 text-xs bg-neon-amber/10 border border-neon-amber/20 rounded-xl px-3 py-2 hover:border-neon-amber/40 transition-all cursor-pointer">
                  <span>⏰</span>
                  <span className="text-white/70">Revisit <span className="text-neon-amber font-semibold">{t.topic}</span></span>
                  <ArrowRight size={10} className="text-neon-amber" />
                </div>
              </Link>
            ))}
          </div>
      ...
      {/* Subject Progress + Daily Goal + Weekly Activity */}
                  ...
                  <div key={subject}>
                    ...
                    <div className="h-2 bg-white/[0.06] rounded-full overflow-hidden">
                      <motion.div initial={{ width:0 }} animate={{ width:`${averageScore}%` }} transition={{ duration:1, delay:0.3 }}
                        className="h-full rounded-full" style={{ background:`linear-gradient(90deg, ${c}80, ${c})` }} />
                    </div>
                  </div>
                );
              })}
          ...
          <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.25 }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {todayQ >= 5 ? <p className="text-xs text-neon-green mt-0.5 font-semibold">✅ Goal done!</p>
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1">
                    <motion.div initial={{ height:0 }} animate={{ height:`${Math.max(4,(v/max)*100)}%` }}
                      ...
                      className="w-full rounded-t-sm min-h-[4px]" style={{ background:v>0 ? '#a78bfa' : 'rgba(255,255,255,0.08)' }} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.3 }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { to:'/app/tutor',          icon:MessageSquare, label:'AI Tutor',      sub:'Get instant help',  color:'#00e5ff' },
              { to:'/app/quiz',           icon:FileQuestion,  label:'Quiz',          sub:'Test knowledge',    color:'#ffb830' },
              ...
              <Link key={to} to={to}>
                <motion.div whileHover={{ y:-2 }} whileTap={{ scale:0.97 }}
                  className="flex flex-col gap-2 p-3.5 rounded-2xl border border-brand-border hover:border-brand-border2 cursor-pointer transition-all"
        ...
        <motion.div initial={{ opacity:0, y:16 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.42 }}
          ...
          <div className="grid grid-cols-3 gap-2">
            {SUBJECTS.map(({ emoji, name, color }) => (
              <Link key={name} to="/app/study-sessions">
                <motion.div whileHover={{ scale:1.05 }} whileTap={{ scale:0.95 }}
                  className="flex flex-col items-center text-center p-2 rounded-xl border border-brand-border hover:border-brand-border2 transition-all cursor-pointer"
                  style={{ background:`${color}08` }}>
          ...
          {(profile?.badges||[]).length === 0 ? (
            <div className="text-center py-4">
              ...
              <div className="flex justify-center gap-2 mt-3 opacity-25">
                {['📝','💯','🏆','⚡','🔥'].map((b,i) => <span key={i} className="text-xl grayscale">{b}</span>)}
              </div>
            </div>
          ) : (
            <div className="flex flex-wrap gap-2">
