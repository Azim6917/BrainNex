        ...
        <div className="space-y-5">
          ...
          <div>
            <label className="text-xs font-medium text-white/50 mb-2 block">
              Topic <span className="text-white/20">(be specific)</span>
            </label>
            <input value={topic} onChange={e => setTopic(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && start()}
              placeholder="e.g. Newton's Laws, Photosynthesis, World War II..."
          ...
          <div>
            <label className="text-xs font-medium text-white/50 mb-2 block">
              Number of Questions: <span className="text-cyan">{numQ}</span>
            </label>
...
function ActiveQuiz({ quiz, onComplete }) {
  ...
  useEffect(() => {
    ...
    timerRef.current = setInterval(() => {
      setTimeLeft(t => {
        if (t <= 1) {
          clearInterval(timerRef.current);
          // Auto-submit as wrong if time runs out and not answered
          handleSelect(-1);
          return 0;
        }
        return t - 1;
  ...
  const handleSelect = (idx) => {
    ...
    setSelected(idx);
    setShowExp(true);
    // Record this answer immediately — one entry per question, no duplicates
    setAnswers(prev => [
      ...prev,
      { questionIndex: current, selectedIndex: idx, correct: idx === q.correctIndex }
    ]);
  };

  const next = () => {
    if (current + 1 >= total) {
      // ── FIX: use the answers array directly — it already has the last answer ──
      // Don't add +1 here — setAnswers is called in handleSelect before next() is ever called
      // (they are separate button clicks), so answers state is already complete.
      const allAnswers = answers; // already includes current question's answer
      const correct    = allAnswers.filter(a => a.correct).length;
      onComplete({ quiz, answers: allAnswers, correct, total });
    } else {
      setSelected(null);
      setShowExp(false);
      setCurrent(c => c + 1);
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-3 text-sm">
        <span className="text-white/40 truncate max-w-xs">{quiz.title}</span>
        <div className="flex items-center gap-4 flex-shrink-0">
          <div className="flex items-center gap-1.5 font-mono text-sm" style={{ color: timerColor }}>
            <Clock size={14} /> {timeLeft}s
          </div>
          <span className="text-white/40">{current + 1}/{total}</span>
        </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={current}
          ...
          className="glass border border-brand-border rounded-2xl p-6 mb-4">
          <p className="text-xs text-cyan font-semibold uppercase tracking-widest mb-3">
          ...
          <div className="space-y-3">
            {q.options.map((opt, i) => {
              ...
              if (selected !== null) {
                ...
                else                             cls = 'border-brand-border text-white/30 opacity-60';
              }
              return (
                <motion.button key={i}
                  ...
                  onClick={() => handleSelect(i)}
                  disabled={selected !== null}
                  className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl border text-sm text-left transition-all disabled:cursor-default ${cls}`}>
                  <span className="w-7 h-7 rounded-lg bg-white/10 flex items-center justify-center text-xs font-bold flex-shrink-0">
                    {['A','B','C','D'][i]}
                  </span>
                  <span className="flex-1">{opt}</span>
                  {selected !== null && i === q.correctIndex && <CheckCircle size={16} className="flex-shrink-0" />}
                  {selected !== null && i === selected && i !== q.correctIndex && <XCircle size={16} className="flex-shrink-0" />}
                </motion.button>
              );
            })}
          </div>
      ...
      <AnimatePresence>
        {showExp && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
            className={`rounded-2xl px-5 py-4 mb-4 border text-sm leading-relaxed ${
      ...
      {selected !== null && (
        <motion.button initial={{ opacity: 0 }} animate={{ opacity: 1 }}
          onClick={next} className="btn-cyan w-full py-3.5 text-sm font-semibold">
          {current + 1 >= total ? 'See Results →' : 'Next Question →'}
        </motion.button>
      )}
    </div>
  ...
  return (
    <div className="max-w-lg mx-auto">
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
        ...
        <p className="text-white/30 text-sm mb-2">{result.quiz.topic} · {result.quiz.difficulty}</p>
        <div className="flex items-center justify-center gap-2 text-neon-amber text-sm font-semibold mb-8">
          <Zap size={15} /> +{xpEarned} XP earned
        </div>
        ...
        <div className="grid grid-cols-3 gap-3 mb-8">
          {[
            ...
            <div key={label} className="bg-white/[0.04] rounded-xl p-3">
              <div className="font-syne font-black text-xl" style={{ color: c }}>{val}</div>
              <div className="text-xs text-white/30 mt-0.5">{label}</div>
            </div>
          ))}
        </div>
        ...
        {/* Per-question breakdown */}
        <div className="space-y-2 mb-8 text-left max-h-48 overflow-y-auto">
          {result.quiz.questions.map((q, i) => {
            const ans     = result.answers[i];
            const correct = ans?.correct ?? false;
            return (
              <div key={i}
                className={`flex items-start gap-2.5 px-3 py-2 rounded-xl text-xs ${correct ? 'bg-neon-green/10' : 'bg-red-500/10'}`}>
                {correct
                  ? <CheckCircle size={13} className="text-neon-green flex-shrink-0 mt-0.5" />
...
export default function QuizPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const h = await fetchQuizHistory(10);
        if (h.data.length > 0) {
          const r = await getAdaptiveDifficulty(h.data.map(q => q.score), profile?.currentDifficulty || 'intermediate');
          setAdaptiveTip(r.data);
        }
      } catch { /* silent */ }
    };
  ...
  return (
    <div className="p-6 lg:p-8 max-w-3xl">
      ...
      {phase === 'setup'   && <QuizSetup onStart={(q, d) => { setQuiz(q); setPhase('active'); }} currentDifficulty={profile?.currentDifficulty} adaptiveTip={adaptiveTip} />}
