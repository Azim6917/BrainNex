...
function NodeCard({ node, onClick }) {
  ...
  return (
    <motion.div
      ...
      whileTap={node.status !== 'locked' ? { scale: 0.97 } : {}}
      onClick={() => node.status !== 'locked' && onClick(node)}
      className={`relative p-4 rounded-2xl border ${bg} ${border} transition-all
        ${node.status === 'locked' ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}
        ${node.status === 'current' ? 'ring-1 ring-cyan/30' : ''}`}
    >
      {node.status === 'current' && (
      ...
      <h3 className={`font-syne font-bold text-sm mb-1 ${node.status === 'locked' ? 'text-white/30' : 'text-white'}`}>
        {node.title}
      </h3>
      <p className="text-xs text-white/40 leading-relaxed mb-3">{node.description}</p>
      <div className="flex items-center justify-between text-[10px] text-white/30">
  ...
  return (
    <div className="relative">
      <button
        type="button"
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between input-dark text-sm py-2.5 px-4"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute top-full left-0 right-0 mt-1 bg-brand-bg3 border border-brand-border2 rounded-xl py-1 z-50 shadow-2xl max-h-56 overflow-y-auto"
          >
            {options.map(opt => (
              <button key={opt} type="button"
                onClick={() => { onChange(opt); setOpen(false); }}
                className={`w-full text-left px-4 py-2.5 text-sm transition-colors hover:bg-white/[0.06]
                  ${opt === value ? 'text-cyan bg-cyan/10' : 'text-white/70'}`}>
                {opt}
              </button>
            ))}
          </motion.div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div>
          ...
          <div className="flex gap-2">
            {LEVELS.map(l => (
              <button key={l} onClick={() => setLevel(l)}
                className={`text-xs px-4 py-2.5 rounded-xl border capitalize transition-all
                  ${l === level ? 'bg-cyan/20 border-cyan/40 text-cyan' : 'border-brand-border text-white/40 hover:border-brand-border2'}`}>
                {l}
              </button>
            ))}
          </div>
        ...
        <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
          ...
          className="btn-cyan flex items-center gap-2 py-2.5 px-6 text-sm disabled:opacity-50">
          {loading
            ? <><div className="w-4 h-4 border-2 border-brand-bg/40 border-t-brand-bg rounded-full animate-spin" /> Generating...</>
            : <><Map size={15} /> Generate Path</>}
        </motion.button>
      </motion.div>
              ...
              <div key={rowIdx}>
                {rowIdx > 0 && (
                  <div className="flex justify-center my-4">
                    <div className="flex flex-col items-center gap-1">
                      {[0,1,2].map(i => <div key={i} className="w-px h-2 bg-brand-border" />)}
        ...
        {selected && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9 }}
              ...
              <div className="flex gap-3">
                <button onClick={() => setSelected(null)} className="flex-1 btn-outline py-2.5 text-sm">Close</button>
                <button onClick={() => setSelected(null)} className="flex-1 btn-cyan py-2.5 text-sm">Start Topic →</button>
              </div>
            </motion.div>
          </motion.div>
        )}
